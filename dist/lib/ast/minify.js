...
function minify(ast, options = {}, recursive = false, errors, nestingContent, context = {}) {
    ...
    if ('chi' in ast && ast.chi.length > 0) {
        ...
        for (; i < ast.chi.length; i++) {
            ...
            if (node.typ == NodeType.AtRuleNodeType && node.nam == 'font-face') {
                continue;
            }
    ...
    if (ast.typ == NodeType.StyleSheetNodeType) {
        ...
        while (parents.length > 0) {
            ...
            for (let k = 0; k < parent.chi.length; k++) {
                ...
                if (!('chi' in node) || node.typ == NodeType.StyleSheetNodeType || (node.typ == NodeType.AtRuleNodeType && node.nam == 'font-face')) {
                    continue;
                }
                // @ts-ignore
