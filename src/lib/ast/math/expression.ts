...
import { LOC, mathFuncs } from "../../syntax/constants.ts";
...
export function evaluate(tokens: Token[]): Token[] {
    ...
    if (
        ...
        mathFuncs.includes((<FunctionToken>tokens[0]).val)
    ) {
        ...
        if (
            ...
            (result[0] as FunctionToken).val === "calc" &&
            (result[0] as FunctionToken).chi.length === 1
        ) {
            return (result[0] as FunctionToken).chi.slice();
        }
    ...
    if (nodes.length <= 1) {
        if (nodes.length == 1) {
            ...
            // @ts-ignore
            if (
                nodes[0].typ == EnumToken.IdenTokenType &&
                // @ts-expect-error
                typeof Math[(<IdentToken>nodes[0]).val.toUpperCase()] == "number"
            ) {
                return [
                    {
                        ...nodes[0],
    ...
    for (i = 0; i < nodes.length; i++) {
        ...
        if (token.typ == EnumToken.Sub) {
            if (!isScalarToken(<Token>nodes[i + 1])) {
                token = {
                    ...
                    [LOC]: { ...nodes[i][LOC], end: nodes[i + 1]![LOC]!.end as number },
                } as ListToken;
            } else {
                token = doEvaluate(
                    nodes[i + 1] as Token,
                    { typ: EnumToken.NumberTokenType, val: -1, [LOC]: nodes[i + 1][LOC] },
                    EnumToken.Mul,
                );
            }
        ...
        if (!map.has(token.typ)) {
            ...
            (map.get(token.typ) as Token[]).push(token);
        }
    }
    ...
    return [...map].reduce((acc: Token[], curr: [EnumToken, Token[]]) => {
        ...
        if (token.typ != EnumToken.BinaryExpressionTokenType) {
            if ("val" in token && +(token as NumberToken).val < 0) {
                acc.push({ typ: EnumToken.Sub, [LOC]: token[LOC] }, {
                    ...token,
                    val: -(token as NumberToken).val,
                    [LOC]: token[LOC],
                } as Token);
                return acc;
            }
    ...
    switch (token.val) {
        ...
        case "hypot": {
            const chi = values.filter(
                (t) =>
                    ![EnumToken.WhitespaceTokenType, EnumToken.CommentTokenType, EnumToken.CommaTokenType].includes(
                        t.typ,
                    ),
            );
        ...
        case "max": {
            const strategy =
                token.val == "round" && values[0]?.typ == EnumToken.IdenTokenType
                    ? (values.shift() as IdentToken).val
                    : null;
    ...
    if (token.typ == EnumToken.BinaryExpressionTokenType) {
        if ([EnumToken.Mul, EnumToken.Div].includes((token as BinaryExpressionToken).op)) {
            ...
            result.push(
                ...
                { typ: (token as BinaryExpressionToken).op, [LOC]: (token as BinaryExpressionToken)[LOC] } as Token,
...
function evaluateExpression(token: Token): Token {
    ...
    if ((token as BinaryExpressionToken).r.typ == EnumToken.BinaryExpressionTokenType) {
        (token as BinaryExpressionToken).r = <BinaryExpressionNode>(
            evaluateExpression((token as BinaryExpressionToken).r)
        );
    }

    if ((token as BinaryExpressionToken).l.typ == EnumToken.BinaryExpressionTokenType) {
        (token as BinaryExpressionToken).l = <BinaryExpressionNode>(
            evaluateExpression((token as BinaryExpressionToken).l)
        );
    }

    return doEvaluate(
        (token as BinaryExpressionToken).l,
        (token as BinaryExpressionToken).r,
        (token as BinaryExpressionToken).op,
    );
}
...
export function buildExpression(tokens: Token[]): BinaryExpressionToken {
    return <BinaryExpressionToken>factor(
        factor(
            tokens.filter((t) => t.typ != EnumToken.WhitespaceTokenType),
            ["/", "*"],
        ),
        ["+", "-"],
    )[0];
...
function factorToken(token: Token): Token {
    if (
        ...
        ((token.typ == EnumToken.MathFunctionTokenType || token.typ == EnumToken.FunctionTokenType) &&
            (token as FunctionToken).val == "calc")
    ) {
        if (
            (token.typ == EnumToken.MathFunctionTokenType || token.typ == EnumToken.FunctionTokenType) &&
            (token as FunctionToken).val == "calc"
        ) {
            token = { ...token, typ: EnumToken.ParensTokenType, [LOC]: token[LOC] } as ParensToken;
