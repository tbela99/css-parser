...
import {definedPropertySettings, EnumToken, expand} from "../ast";
...
export function getDefaultRenderOptions(options: RenderOptions = {}) {
...
function updateSourceMap(node: AstRuleList | AstComment, options: RenderOptions, sourcemap: SourceMap, position: Position, str: string) {
...
export function renderToken(token: Token, options: RenderOptions = {}, reducer?: (acc: string, curr: Token) => string, errors?: ErrorDescription[]): string {
    ...
    switch (token.typ) {
        ...
        case EnumToken.ListToken:
            ...
            return token.chi.reduce((acc: string, curr: Token) => acc + renderToken(curr, options), '');
        ...
        case EnumToken.BinaryExpressionTokenType:
            ...
            return renderToken(token.l, options) + (token.op == EnumToken.Add ? ' + ' : (token.op == EnumToken.Sub ? ' - ' : (token.op == EnumToken.Mul ? '*' : '/'))) + renderToken(token.r, options);
            ...
            if (options.convertColor) {
                ...
                if (token.cal != null) {
                    ...
                    return clamp(token).val + '(' + (slice ? (<Token[]>token.chi).slice(0, -2) : <Token[]>token.chi).reduce((acc: string, curr: Token): string => {
                        ...
                        const val: string = renderToken(curr, options);
                ...
                if (Array.isArray(token.chi) && token.chi.some((t: Token): boolean => t.typ == EnumToken.FunctionTokenType || (t.typ == EnumToken.ColorTokenType && Array.isArray(t.chi)))) {
                    ...
                    return (token.val.endsWith('a') ? token.val.slice(0, -1) : token.val) + '(' + token.chi.reduce((acc: string, curr: Token) => acc + (acc.length > 0 && !(acc.endsWith('/') || curr.typ == EnumToken.LiteralTokenType) ? ' ' : '') + renderToken(curr, options), '') + ')';
                }
        ...
        case EnumToken.GridTemplateFuncTokenType:
            ...
            if (
                ...
                // calc(200px) => 200px
                return token.chi.reduce((acc: string, curr: Token) => acc + renderToken(curr, options, reducer), '')
            }
        ...
        case EnumToken.ResolutionTokenType:
            ...
            let val: string = (<FractionToken>token.val).typ == EnumToken.FractionTokenType ? renderToken(<FractionToken>token.val, options) : reduceNumber(<string | number>token.val);
        ...
        case EnumToken.PercentageTokenType:
            ...
            const perc: string = (<FractionToken>token.val).typ == EnumToken.FractionTokenType ? renderToken(<FractionToken>token.val, options) : reduceNumber(<string>token.val);
            return options.minify && perc == '0' ? '0' : (perc.includes('/') ? perc.replace('/', uni + '/') : perc + uni);

        case EnumToken.NumberTokenType:

            return (<FractionToken>token.val).typ == EnumToken.FractionTokenType ? renderToken(<FractionToken>token.val, options) : reduceNumber(<string>token.val);
