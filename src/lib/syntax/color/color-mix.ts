...
export function colorMix(
    colorSpace: IdentToken,
    hueInterpolationMethod: IdentToken | null,
    color1: ColorToken,
    percentage1: PercentageToken | NumberToken | null,
    color2: ColorToken,
    ...
    if (
        ...
        (components1[3] as IdentToken).val == "none" &&
        values2.length == 4
    ) {
        values1[3] = values2[3];
    }
    ...
    if (
        ...
        (components2[3] as IdentToken).val == "none" &&
        values1.length == 4
    ) {
        values2[3] = values1[3];
    }
    ...
    if (lchSpaces.includes(colorSpace1) || lchSpaces.includes(colorSpace.val)) {
        if (
            (components1[2].typ == EnumToken.IdenTokenType && (components1[2] as IdentToken).val == "none") ||
            values1[2] == 0
        ) {
            values1[2] = values2[2];
        }
    ...
    if (lchSpaces.includes(colorSpace2) || lchSpaces.includes(colorSpace.val)) {
        if (
            (components2[2].typ == EnumToken.IdenTokenType && (components2[2] as IdentToken).val == "none") ||
            values2[2] == 0
        ) {
            values2[2] = values1[2];
        }
    ...
    if (hueInterpolationMethod != null) {
        ...
        const [h1, h2] = interpolateHue(
            ...
            values1[hueIndex] * multiplier,
            values2[hueIndex] * multiplier,
        );
    ...
    switch (colorSpace.val) {
        ...
        case "xyz-d65":
        case "xyz-d50":
            let values: number[] = (<number[]>values1)
                .map((v1: number, i: number) => (mul1 * v1 * p1 + mul2 * values2[i] * p2) / mul)
                .concat(mul == 1 ? [] : [mul]);
