                ...
                if (!nested) {
                    return {
                        ...
                        errors: [
                            {
                                action: "drop",
                                message: `Unexpected token ${EnumToken[stream[i].typ]} at ${stream[i][LOC]!.srcId}:${stream[i][LOC]!.sta.lin}:${
                ...
                if (nested === false && !options.nestedRule) {
                    return {
                        ...
                        errors: [
                            {
                                action: "drop",
                                message: `Nesting selector is not allowed at ${token[LOC]!.srcId}:${token[LOC]!.sta.lin}:${
                                    token[LOC]!.sta.col
                                }`,
                                node: token,
                                location: token[LOC],
                ...
                if (stack.length > 0 && nodes.includes(stack.at(-1)?.typ)) {
                    return {
                        ...
                        errors: [
                            {
                                action: "drop",
                                message: `Unexpected combinator ${EnumToken[token.typ]} at ${token[LOC]!.srcId}:${token[LOC]!.sta.lin}:${
                                    token[LOC]!.sta.col
                                }`,
                                node: token,
                                location: token[LOC],
                ...
                if (stack.at(-1)?.typ !== EnumToken.AttrStartTokenType) {
                    return {
                        ...
                        errors: [
                            {
                                action: "drop",
                                message: `Unexpected token ${EnumToken[token.typ]} at ${token[LOC]!.srcId}:${token[LOC]!.sta.lin}:${
                                    token[LOC]!.sta.col
                                }`,
                                node: token,
                                location: token[LOC],
                    ...
                    if (
                        ...
                        return {
                            ...
                            errors: [
                                {
                                    action: "drop",
                                    message: `Unexpected token ${EnumToken[slice[0].typ]} at ${slice[0][LOC]!.srcId}:${slice[0][LOC]!.sta.lin}:${
                                        slice[0][LOC]!.sta.col
                                    }`,
                                    node: slice[0],
                                    location: slice[0][LOC],
            ...
            default:
                return {
                    ...
                    errors: [
                        {
                            action: "drop",
                            message: `Unsupported selector token ${EnumToken[token.typ]} at ${token[LOC]!.srcId}:${
                                token[LOC]!.sta.lin
                            }:${token[LOC]!.sta.col}`,
                            node: token,
                            location: token[LOC]!,
    ...
    if (stack.length > 0) {
        return {
            ...
            errors: [
                {
                    action: "drop",
                    message: `Unmatched token ${EnumToken[stack.at(-1)!.typ]} at ${stack.at(-1)![LOC]!.srcId}:${
