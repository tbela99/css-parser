...
import {AstAtRule, AstDeclaration, AstNode, AstRule, OptimizedSelector, ParserOptions} from "../../@types";
...
export function deduplicate(ast: AstNode, options: ParserOptions = {}, recursive: boolean = false): AstNode {
    // @ts-ignore
    if (('chi' in ast) && ast.chi?.length > 0) {
                    ...
                    if (wrapper != null) {
                        ...
                        while (i < ast.chi.length) {
                            ...
                            if (nextNode.raw[0].length == 0 ||
                                ...
                                if (hasOnlyDeclarations(<AstRule>wrapper)) {
                                    // @ts-ignore
                                    wrapper.chi.push(...(<AstRule>nextNode).chi);
                                    // @ts-ignore
                                    ast.chi.splice(i, 1);
                                    continue;
                                }
                            ...
                            // @ts-ignore
                            wrapper.chi.push(<AstRule>nextNode);
                            // @ts-ignore
                            ast.chi.splice(i, 1);
                            node = wrapper;
                        }
        ...
        if (recursive && node != null && ('chi' in node)) {
            ...
            if (node.chi.some(n => n.typ == 'Declaration')) {
                deduplicateRule(<AstRule | AstAtRule>node);
            }
            else {
                // @ts-ignore
                if (!(node.typ == 'AtRule' && (<AstAtRule>node).nam != 'font-face')) {
...
export function deduplicateRule(ast: AstRule | AstAtRule): AstRule | AstAtRule {
    // @ts-ignore
...
function reduceRawTokens(raw: string[][]) {
...
function trimRawToken(raw: string[]) {
    while (raw.length > 0) {
        if (raw[0] == ' ') {
            raw.shift();
            continue;
...
function reducer(acc: string[], curr: string[]) {
    acc.push(curr.join(''));
    return acc;
}
