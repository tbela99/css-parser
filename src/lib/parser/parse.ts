...
function parseTokens(tokens: Token[], options: ParseTokenOptions = {}): Token[] {
        ...
        if (funcLike.includes(t.typ)) {
            ...
            if (t.typ == 'UrlFunc') {
                ...
                if (t.chi[0]?.typ == 'String') {
                    ...
                    if ((<UrlToken>t.chi[0]).val.slice(1, 5) != 'data:' && urlTokenMatcher.test(value)) {
                        ...
                        // @ts-ignore
                        (<UrlToken>t.chi[0]).val = options.src !== '' && options.resolveUrls ? options.resolve(value, options.src).absolute : value;
                    }
                }
            ...
            if (t.chi.length > 0) {
                ...
                if (t.typ == 'Pseudo-class-func' && (<PseudoClassFunctionToken>t).val == ':is' && options.compress) {
                    ...
                    //
                    const count: number = (<PseudoClassFunctionToken>t).chi.filter(t => t.typ != 'Comment').length;
                    if (count == 1 ||
                        (i == 0 &&
