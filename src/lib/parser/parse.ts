import {
    ...
    ParserOptions, Position, PseudoClassFunctionToken, PseudoClassToken, StringToken, Token, UrlToken
} from "../../@types";
import {
    isAtKeyword,
    isDimension,
            ...
            if (atRule.val == 'import') {
                ...
                if (options.processImport) {
                    ...
                    try {
                        ...
                        // @ts-ignore
                        const root: ParseResult = await options.load((<UrlToken>tokens[0]).val.slice(1, -1), <string>options.src).
                        then(src =>
                            parse(src, Object.assign({}, options, {src: (<UrlToken>tokens[0]).val.slice(1, -1)})
                            )
                        );
...
function parseTokens(tokens: Token[], options: { parseColor?: boolean, compress?: boolean } = {}): Token[] {
    ...
    for (let i = 0; i < tokens.length; i++) {
        ...
        if (t.typ == 'Attr-start') {
            ...
            Object.assign(t, <AttrToken>{typ: 'Attr', chi: tokens.splice(i + 1, k - i)});

            // @ts-ignore
            if ((<AttrToken>t).chi.at(-1).typ == 'Attr-end') {

                // @ts-ignore
                (<AttrToken>t).chi.pop();

                // @ts-ignore
                if ((<AttrToken>t).chi.length > 1) {
        ...
        if (funcLike.includes(t.typ)) {
            ...
            if (t.chi.length > 0) {
                ...
                if (t.typ == 'Pseudo-class-func' && (<PseudoClassFunctionToken>t).val == ':is' && options.compress) {
                    //
                    //     console.debug({is: t});
                    const count = (<PseudoClassFunctionToken>t).chi.filter(t => t.typ != 'Comment').length;
                    if (count == 1 ||
                        (i == 0 &&
                        ...
                        tokens.splice(i, 1, ...(<PseudoClassFunctionToken>t).chi);
