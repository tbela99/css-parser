...
const dir = dirname$1(new URL(import.meta.url).pathname) + '/../files';
describe('parse block', function () {
    it('parse file', async function () {
        const file = (await readFile(`${dir}/css/smalli.css`)).toString();
        transform(file).then(result => f(result.ast).deep.equals(JSON.parse((readFileSync(dir + '/json/smalli.json')).toString())));
    });
    it('parse file #2', async function () {
        const file = (await readFile(`${dir}/css/small.css`)).toString();
        transform(file).then(result => f(result.ast).deep.equals(JSON.parse((readFileSync(dir + '/json/small.json')).toString())));
    });
    it('parse file #3', async function () {
        const file = (await readFile(`${dir}/css/invalid-1.css`)).toString();
        transform(file).then(result => f(result.ast).deep.equals(JSON.parse((readFileSync(dir + '/json/invalid-1.json')).toString())));
    });
    it('parse file #4', async function () {
        const file = (await readFile(`${dir}/css/invalid-2.css`)).toString();
        transform(file).then(result => f(result.ast).deep.equals(JSON.parse((readFileSync(dir + '/json/invalid-2.json')).toString())));
    });
